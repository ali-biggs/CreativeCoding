...
const sketch = () => {
  return ({ context, width, height, frame }) => {
    ...
    for (i = 0; i < numCells; i++) {
      ...
      //const n = random.noise2D(x + frame * 10, y, params.frequency); //returns a number between -1 to 1
      const n = random.noise3D(x, y, f * 10, params.frequency);

      const angle = n * Math.PI * params.amplitude;

      //const scale = (n + 1) / 2 * 30;
      //const scale = (n * 0.5 + 0.5) * 30; same result as above
      const scale = math.mapRange(n, -1, 1, params.scaleMin, params.scaleMax); //also same as above
